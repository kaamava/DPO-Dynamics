...
try:
    ...
    print("Warning: wandb not available. Training monitoring will be limited.")
...
# Import BERTScore for semantic similarity computation
try:
    from bert_score import score as bert_score_fn
    BERT_SCORE_AVAILABLE = True
except ImportError:
    BERT_SCORE_AVAILABLE = False
    print("Warning: bert-score not available. Semantic distance will not be computed.")
...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[str, Any]:
        ...
        # Collect text data for semantic similarity computation
        chosen_texts = [example.get("chosen_text", example.get("chosen", "")) for example in examples]
        rejected_texts = [example.get("rejected_text", example.get("rejected", "")) for example in examples]
        ...
        # Add text data to output
        output["chosen_texts"] = chosen_texts
        output["rejected_texts"] = rejected_texts
...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if args.generate_during_eval and not is_comet_available():
            raise ValueError(
                "`generate_during_eval=True` requires Comet to be installed."
                " Please install `comet-ml` to resolve."
            )
    ...
    def dpo_loss(
        ...
        chosen_hidden_states: Optional[torch.FloatTensor] = None,
        rejected_hidden_states: Optional[torch.FloatTensor] = None,
        chosen_texts: Optional[list[str]] = None,
        ...
        if self.loss_type == "sigmoid":
        ...
        elif self.loss_type == "last_layer_dpo":
            # 初始化uncertainty_bonus为0，确保始终应用ENTK计算
            uncertainty_bonus = torch.zeros_like(logits)
            
            if chosen_hidden_states is not None and rejected_hidden_states is not None:
                chosen_hidden_float = chosen_hidden_states.float()
                rejected_hidden_float = rejected_hidden_states.float()
                reference_vector = self.compute_reference_vector(chosen_hidden_float, rejected_hidden_float)
                sigma_matrix = self.compute_covariance_matrix(chosen_hidden_float, rejected_hidden_float)

                try:
                    sigma_inv = torch.linalg.inv(sigma_matrix)
                except torch._C._LinAlgError as e:
                    print(f"Warning: Singular matrix detected, using pseudo-inverse. Error: {e}")
                    sigma_inv = torch.linalg.pinv(sigma_matrix)
                    print(f"Warning: Singular matrix detected, using pseudo-inverse. Error: {e}")
                    

                gamma_chosen = self.compute_uncertainty_bonus(chosen_hidden_float, reference_vector, sigma_inv)
                gamma_rejected = self.compute_uncertainty_bonus(rejected_hidden_float, reference_vector, sigma_inv)  
                uncertainty_bonus = gamma_chosen - gamma_rejected
                uncertainty_bonus = torch.clamp(uncertainty_bonus, min=-5.0, max=5.0)
                                    
            # 始终计算ENTK logits，即使uncertainty_bonus为0也保持ENTK结构
            entk_logits = self.beta * logits + uncertainty_bonus 
            ...
            losses = (
                -F.logsigmoid(entk_logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-entk_logits) * self.label_smoothing
            )
            print(f"Losses: {losses}")
        
        elif self.loss_type == "entk_dpo":
            uncertainty_bonus = torch.zeros_like(logits)
            chosen_grad_features, rejected_grad_features = self.compute_gradient_features_from_text(chosen_texts, rejected_texts)
            reference_vector = self.compute_reference_vector(chosen_grad_features, rejected_grad_features)
            sigma_matrix = self.compute_covariance_matrix(chosen_grad_features, rejected_grad_features)
            ...
            gamma_chosen = self.compute_uncertainty_bonus(chosen_grad_features, reference_vector, sigma_inv)
            gamma_rejected = self.compute_uncertainty_bonus(rejected_grad_features, reference_vector, sigma_inv)
            uncertainty_bonus = gamma_chosen - gamma_rejected
            print(f"uncertainty_bonus: {uncertainty_bonus}")
            entk_logits = self.beta * logits + uncertainty_bonus
            losses = (
            ...
            raise ValueError(
                ...
                "'nca_pair', 'robust', 'bco_pair', 'sppo_hard', 'aot', 'aot_pair', 'discopop', 'apo_zero', 'apo_down', 'ENTK_dpo']"
            )
    ...
    def compute_gradient_features_from_text(
        self, 
        chosen_texts: list[str], 
        rejected_texts: list[str]) -> tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        
        tokenizer = self.processing_class
        device = self.accelerator.device
            
        chosen_grad_features = self._compute_text_gradient_features(chosen_texts, "chosen", tokenizer, device)
        rejected_grad_features = self._compute_text_gradient_features(rejected_texts, "rejected", tokenizer, device)
            
        return chosen_grad_features, rejected_grad_features
    
    def _compute_g_theta_function(self, model, inputs):
        """
        计算 g_θ(s,a) = Σ f_θ(s, a_{1:i-1}, a_i) - Σ ln Σ exp(f_θ(s, a_{1:i}, b))
        ...
        # 第一项：Σ f_θ(s, a_{1:i-1}, a_i)
        # 这是给定前缀条件下，实际选择的token的logit分数
        first_term = 0.0
        
        # 第二项：Σ ln Σ exp(f_θ(s, a_{1:i}, b))  
        # 这是log-sum-exp正则化项
        second_term = 0.0
        
        for i in range(1, seq_len):  # 从第二个token开始（第一个通常是BOS或prompt的一部分）
            if attention_mask[0, i] == 0:  # 跳过填充token
                continue
                
            # 第一项：实际token的logit分数
            actual_token = input_ids[0, i]
            if actual_token < vocab_size:  # 确保token有效
                token_logit = logits[0, i-1, actual_token]  # 使用前一个位置的logit预测当前token
                first_term += token_logit
            
            # 第二项：log-sum-exp正则化
            # 注意：这里我们计算到i-1位置的log-sum-exp，因为公式中是到n-1
            if i < seq_len - 1:  # 只计算到n-1
                logit_slice = logits[0, i-1, :]  # [vocab_size]
                log_sum_exp = torch.logsumexp(logit_slice, dim=0)
                second_term += log_sum_exp
        
        # 计算 g_θ(s,a)
        g_theta = first_term - second_term
        
        return g_theta

    def _compute_text_gradient_features(
        self, 
        texts: list[str], 
        response_type: str, 
        tokenizer, 
        ...
        """
        计算符合NTK理论的梯度特征 φ(s,a) = ∇_θ g_θ(s,a)
        其中 g_θ(s,a) = Σ f_θ(s, a_{1:i-1}, a_i) - Σ ln Σ exp(f_θ(s, a_{1:i}, b))
        """
        ...
        # 保存当前训练状态
        original_training_mode = unwrapped_model.training
        unwrapped_model.eval()  # 设为评估模式避免dropout影响
        ...
        for i, text in enumerate(texts):
            ...
            with torch.enable_grad():
                ...
                # 计算正确的 g_θ(s,a) 函数
                g_theta = self._compute_g_theta_function(unwrapped_model, inputs)
            
                # 使用autograd.grad计算梯度
                gradients_list = torch.autograd.grad(
                    outputs=g_theta,
                    inputs=list(unwrapped_model.parameters()),
                    create_graph=False,
                    retain_graph=False,
                    allow_unused=True
                )
                ...
                for grad in gradients_list:
                    if grad is not None:
                        flat_grad = grad.flatten().detach().to(dtype)  # 转换为float32
                        total_params += flat_grad.numel()
                        gradients.append(flat_grad)
    
                grad_vector = torch.cat(gradients)
                # 使用平均池化压缩特征
                if grad_vector.numel() > target_feature_dim:

                    pool_size = grad_vector.numel() // target_feature_dim
                    grad_vector = grad_vector[:pool_size * target_feature_dim]
                    grad_vector = grad_vector.view(-1, pool_size).mean(dim=1)
                elif grad_vector.numel() < target_feature_dim:
 
                    padded = torch.zeros(target_feature_dim, dtype=dtype, device=device)
                    padded[:grad_vector.numel()] = grad_vector
                    grad_vector = padded
                gradient_features.append(grad_vector)
                    
        # 恢复训练状态
        unwrapped_model.train(original_training_mode)
        

        gradient_features = torch.stack(gradient_features, dim=0)
        gradient_features = F.normalize(gradient_features, p=2, dim=1)
    ...
    def compute_uncertainty_bonus(self, hidden_states: torch.Tensor, reference_vector: torch.Tensor, sigma_inv: torch.Tensor) -> torch.Tensor:
        """
        bonus Γ(s,a) = ||φ(s,a) - ν||_{Σ_D^{-1}}
        
        Args:
            hidden_states: Shape [batch_size, hidden_size] - φ(s,a)
            reference_vector: Shape [hidden_size] - ν 
            sigma_inv: Shape [hidden_size, hidden_size] - Σ_D^{-1}
            
        Returns:
            uncertainty_bonus: Shape [batch_size] - Γ(s,a)
        """
        # 确保所有张量具有相同的数据类型和设备
        hidden_states = hidden_states.to(dtype=sigma_inv.dtype, device=sigma_inv.device)
        reference_vector = reference_vector.to(dtype=sigma_inv.dtype, device=sigma_inv.device)
        
        # compute φ(s,a) - ν
        diff = hidden_states - reference_vector.unsqueeze(0)  # [batch_size, hidden_size]
        
        # (φ(s,a) - ν)^T Σ_D^{-1} (φ(s,a) - ν)
        # ||φ(s,a) - ν||_{Σ_D^{-1}}^2
        mahalanobis_sq = torch.sum(diff @ sigma_inv * diff, dim=1)  # [batch_size]
        
        uncertainty_bonus = torch.sqrt(mahalanobis_sq + 1e-8)  
        
        return uncertainty_bonus


    def compute_covariance_matrix(self, chosen_hidden_states: torch.Tensor, rejected_hidden_states: torch.Tensor, lambda_reg: float = 1e-2) -> torch.Tensor:
        """
        Σ_D = λI + Σ(φ(s,a+) - φ(s,a-))(φ(s,a+) - φ(s,a-))^T
        
        Args:
            chosen_hidden_states: Shape [batch_size, hidden_size]
            rejected_hidden_states: Shape [batch_size, hidden_size] 
            lambda_reg: 正则化参数 λ
            
        Returns:
            sigma_matrix: Shape [hidden_size, hidden_size]
        """
        diff = chosen_hidden_states - rejected_hidden_states  # [batch_size, hidden_size]
        
        covariance = torch.matmul(diff.T, diff) / diff.shape[0]  # [hidden_size, hidden_size]
        
        hidden_size = chosen_hidden_states.shape[1]
       
        identity = torch.eye(hidden_size, device=chosen_hidden_states.device, dtype=chosen_hidden_states.dtype)
        sigma_matrix = lambda_reg * identity + covariance
        
        return sigma_matrix
    
    def compute_reference_vector(self, chosen_hidden_states: torch.Tensor, rejected_hidden_states: torch.Tensor) -> torch.Tensor:
        """
        ν = E[φ(s,a)]
        Args:
            chosen_hidden_states: Shape [batch_size, hidden_size]
        ...
        Returns:
            reference_vector: Shape [hidden_size]
        """
        all_hidden_states = torch.cat([chosen_hidden_states, rejected_hidden_states], dim=0)
        reference_vector = torch.mean(all_hidden_states, dim=0)  # [hidden_size]
    ...
    def concatenated_forward(
        ...
        # Extract hidden states for chosen and rejected
        if hasattr(outputs, 'hidden_states') and outputs.hidden_states is not None:
            # Get the last layer hidden states
            last_hidden_states = outputs.hidden_states[-1]  # shape: (batch_size, seq_len, hidden_size)
        
            
            chosen_hidden_all = last_hidden_states[:num_examples]  # [num_examples, seq_len, hidden_size]
            rejected_hidden_all = last_hidden_states[num_examples:]  # [num_examples, seq_len, hidden_size]
            chosen_mask = loss_mask[:num_examples]  # [num_examples, seq_len]
            rejected_mask = loss_mask[num_examples:]  # [num_examples, seq_len]
            # Get the last valid token's hidden state using vectorized operations
            # Create position indices for each sequence
            seq_len = chosen_mask.shape[1]
            positions = torch.arange(seq_len, device=chosen_mask.device).unsqueeze(0).expand(num_examples, -1)
            
            # For each sequence, find the maximum position where mask is True
            # Use a large negative value for False positions so they don't interfere
            chosen_positions = torch.where(chosen_mask, positions, torch.tensor(-1, device=chosen_mask.device))
            rejected_positions = torch.where(rejected_mask, positions, torch.tensor(-1, device=rejected_mask.device))
            
            # Get the maximum position (last True position) for each sequence
            chosen_last_pos = chosen_positions.max(dim=1)[0]  # [num_examples]
            rejected_last_pos = rejected_positions.max(dim=1)[0]  # [num_examples]
            
            # Use advanced indexing to get hidden states at last valid positions
            batch_indices = torch.arange(num_examples, device=chosen_mask.device)
            chosen_hidden_states = chosen_hidden_all[batch_indices, chosen_last_pos]
            rejected_hidden_states = rejected_hidden_all[batch_indices, rejected_last_pos]
 
            output["chosen_hidden_states"] = chosen_hidden_states
            output["rejected_hidden_states"] = rejected_hidden_states
    ...
    def get_batch_loss_metrics(
        ...
        if self.args.use_liger_loss:
            ...
            losses, chosen_rewards, rejected_rewards = self.dpo_loss(
                ...
                ref_chosen_logps, 
                ref_rejected_logps,
                model_output.get("chosen_hidden_states"),
                model_output.get("rejected_hidden_states"),
                batch.get("chosen_texts"),
                batch.get("rejected_texts")
            )
    ...
    def _save_checkpoint(self, model, trial):
        ...
        model_name_with_loss = f"{model_name}-{self.loss_type}"
        self.create_model_card(model_name=model_name_with_loss)
